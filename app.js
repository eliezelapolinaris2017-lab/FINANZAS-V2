...
const DEFAULT_STATE = {
  ...
  quotes: [],
  reconciliations: [],
  /** ⬇️ NUEVO: Retenciones por empleado (ISR/SS/Otras) */
  withholdings: [], // {id,paymentId,date,employee,ISR,SS,Other,total,status:'Pendiente'|'Pagado',paidDate,note}
  _cloud: { updatedAt: 0 }
};
...
/* ======= NUEVO: utilidades de Retenciones ======= */
function withholdingTotalFromPayment(p){
  return Number(p.retISR||0)+Number(p.retSS||0)+Number(p.retOther||0);
}
function upsertWithholdingFromPayment(p){ // crea/actualiza 1 registro por pago
  const total = withholdingTotalFromPayment(p);
  const existing = state.withholdings.find(w=>w.paymentId===p.id);
  if(total<=0){
    // si no hay retenciones, y existía, elimínalo
    if(existing){ state.withholdings = state.withholdings.filter(w=>w.paymentId!==p.id); }
    return;
  }
  if(existing){
    existing.date = p.date||existing.date;
    existing.employee = p.to||existing.employee;
    existing.ISR = Number(p.retISR||0);
    existing.SS = Number(p.retSS||0);
    existing.Other = Number(p.retOther||0);
    existing.total = existing.ISR+existing.SS+existing.Other;
    // status/paidDate permanecen
  }else{
    state.withholdings.push({
      id: uid(),
      paymentId: p.id,
      date: p.date,
      employee: p.to,
      ISR: Number(p.retISR||0),
      SS: Number(p.retSS||0),
      Other: Number(p.retOther||0),
      total,
      status: 'Pendiente',
      paidDate: null,
      note: ''
...
function renderPayments(){
  ...
  $$('#paymentsTable [data-del]').forEach(b=> b.onclick=()=>{ 
    ...
    // borrar retención asociada
    state.withholdings = state.withholdings.filter(w=>w.paymentId!==id);
    state.payments = state.payments.filter(x=>x.id!==id); 
    save(); toast('Pago eliminado'); 
  });
...
function editPayment(id){
  ...
  upsertWithholdingFromPayment(p);
...
function wirePayments(){
  ...
  $('#paymentForm')?.addEventListener('submit',(ev)=>{
    ...
    // ⬇️ Registrar/actualizar retención ligada a este pago
    upsertWithholdingFromPayment(rec);
    save(); toast('Pago guardado'); ev.target.reset(); payrollBindRetentionInputs();
  });
...
/* ======= NUEVO: Ingresos ajustados por Retenciones =======
   • Suman retenciones PENDIENTES dentro del período (lo que aún no se ha pagado al fisco)
   • Restan retenciones PAGADAS en el período (fecha de pago)
*/
function sumWithholdingsAdd(from,to){
  return state.withholdings
    .filter(w=>w.status!=='Pagado' && inRange(w.date,from,to))
    .reduce((a,b)=>a+Number(b.total||0),0);
}
function sumWithholdingsPaid(from,to){
  return state.withholdings
    .filter(w=>w.status==='Pagado' && inRange(w.paidDate||w.date,from,to))
    .reduce((a,b)=>a+Number(b.total||0),0);
}
function sumIncomesAdj(from,to){
  const base = sumRange(state.incomesDaily, from, to);
  const add  = sumWithholdingsAdd(from,to);
  const sub  = sumWithholdingsPaid(from,to);
  return base + add - sub;
}
...
function renderReports(){
  ...
  const incToday=sumIncomesAdj(today, today);
  const incWeek=sumIncomesAdj(weekStart, today);
  const incMonth=sumIncomesAdj(monthStart, today);
  const incYear=sumIncomesAdj(yearStart, today);
...
/* ======== Inicio con balance corrido (YTD) — no se resetea mes a mes ======== */
function renderHome(){
  const now=new Date();
  const today=now.toISOString().slice(0,10);
  const yearStart=new Date(now.getFullYear(), 0, 1).toISOString().slice(0,10);

  // YTD con ingresos ajustados por retenciones
  const incYTD = sumIncomesAdj(yearStart, today);
  ...
  if($('#kpiIncomesMonth')){ $('#kpiIncomesMonth').textContent = fmt(incYTD); $('#kpiIncomesMonth').title='Ingresos (YTD, ajustado retenciones)'; }
  if($('#kpiExpensesMonth')){ $('#kpiExpensesMonth').textContent = fmt(totalExpYTD); $('#kpiExpensesMonth').title='Gastos (YTD)'; }
  if($('#kpiBalanceMonth')){ $('#kpiBalanceMonth').textContent = fmt(balanceYTD); $('#kpiBalanceMonth').title='Balance (YTD)'; }

  // Gráfico 12 meses — ingresos ajustados
  ...
  for(let i=11;i>=0;i--){
    ...
    const incM=sumIncomesAdj(from, to);
  ...
  reader.onload=()=>{
    try{
      ...
      if(confirm('¿Reemplazar TODO con el archivo? (Cancelar = fusionar)')){
        ...
        ['expensesDaily','incomesDaily','payments','ordinary','budgets','personal','invoices','quotes','reconciliations','withholdings'].forEach(k=>{
...
async function generatePDF(view="expenses", optionalId=null){
  ...
  const titleMap={ payments:"PAGO DE NÓMINA", invoices:"FACTURAS", quotes:"COTIZACIONES", reconciliations:"CONCILIACIÓN BANCARIA", withholdings:"RETENCIONES" };
  ...
  else if(view==="withholdings"){ headers=["Fecha","Empleado","ISR","SS","Otras","Total","Estado","Pagada"]; rows=state.withholdings.map(w=>[w.date,w.employee,Number(w.ISR||0).toFixed(2),Number(w.SS||0).toFixed(2),Number(w.Other||0).toFixed(2),Number(w.total||0).toFixed(2),w.status||'',w.paidDate||'']); total=state.withholdings.reduce((a,w)=>a+Number(w.total||0),0); }
...
/* ===================== RETENCIONES — Vista/acciones ===================== */
function renderWithholdings(){
  // Totales
  const totPend = state.withholdings.filter(w=>w.status!=='Pagado').reduce((a,b)=>a+Number(b.total||0),0);
  const totPaid = state.withholdings.filter(w=>w.status==='Pagado').reduce((a,b)=>a+Number(b.total||0),0);
  $('#whSumPending') && ($('#whSumPending').textContent = fmt(totPend));
  $('#whSumPaid')    && ($('#whSumPaid').textContent    = fmt(totPaid));
  $('#whSumAll')     && ($('#whSumAll').textContent     = fmt(totPend+totPaid));

  // Tabla
  const tbody = $('#withholdingsTable tbody');
  if(!tbody) return; tbody.innerHTML='';
  state.withholdings.slice().sort(byDateDesc).forEach(w=>{
    const tr=document.createElement('tr');
    tr.innerHTML=`
      <td>${w.date||''}</td>
      <td>${w.employee||''}</td>
      ...
      <td class="row-actions">
        ${w.status==='Pagado'
          ? `<button class="btn-outline" data-unpay="${w.id}">Desmarcar</button>`
          : `<button class="btn-outline" data-pay="${w.id}">Pagar</button>`}
  ...
  // wire acciones
  $$('#withholdingsTable [data-pay]').forEach(b=> b.onclick=()=>{
    const w = state.withholdings.find(x=>x.id===b.dataset.pay); if(!w) return;
    w.status='Pagado'; w.paidDate=todayStr(); save(); toast('Retención marcada como pagada');
  });
  $$('#withholdingsTable [data-unpay]').forEach(b=> b.onclick=()=>{
    const w = state.withholdings.find(x=>x.id===b.dataset.unpay); if(!w) return;
    w.status='Pendiente'; w.paidDate=null; save(); toast('Retención marcada como pendiente');
  });
  $$('#withholdingsTable [data-del]').forEach(b=> b.onclick=()=>{
    const id=b.dataset.del;
    // si está ligada a un pago, solo eliminas la retención (no el pago)
    state.withholdings = state.withholdings.filter(x=>x.id!==id);
    save(); toast('Retención eliminada');
  });
}
...
/* =========================================================
   FIX TOTALES: Conciliación (matching) + Nómina (ya existente)
   ========================================================= */
(function () {
  ...
  function updatePayrollSummaryFromTable() {
    try {
      ...
      [...tbody.rows].forEach((row) => {
        ...
        total += neto;
        if (estadoCell.startsWith("pagad")) paid += neto;
      });
...
(function () {
  ...
  function showLoginOverlay() {
    ...
    try { localStorage.removeItem("pinVerified"); sessionStorage.clear(); } catch (_) {}
    const pin = $("#loginPIN"); if (pin) setTimeout(() => pin.focus(), 50);
  }
  ...
  if (logoutBtn) {
    logoutBtn.addEventListener("click", async () => {
      ...
      } catch (e) { console.warn("signOut Firebase opcional falló:", e); }
      showLoginOverlay();
    });
  }
...
/* =========================================================
   Conciliación: contar Coincidencias / No encontradas (ya existente)
   ========================================================= */
(function () {
  const $ = (id) => document.getElementById(id);
  function isMatch(text) {
    const t = String(text || "").toLowerCase().replace(/\s+/g, " ").trim();
    const positives = ["✔","✓","match","coincide","coincidencia","encontrado","found","ok","true","sí","si"];
    const negatives = ["✖","x","no coincide","sin coincidencia","no encontrado","no match","no","false","pendiente","-"];
  ...
  function updateReconMatchSummaryFromTable() {
    ...
    [...tbody.rows].forEach((row) => {
      ...
      totalCsv += monto;
      if (isMatch(matchText)) matches++; else noMatch++;
    });
    if ($("matchCount")) $("matchCount").textContent = String(matches);
    if ($("unmatchedCount")) $("unmatchedCount").textContent = String(noMatch);
    if ($("reconImpTotal")) $("reconImpTotal").textContent = (typeof window.fmt === "function") ? window.fmt(totalCsv)
  ...
  document.addEventListener("DOMContentLoaded", () => { wireReconObservers(); setTimeout(updateReconMatchSummaryFromTable, 0); });
  window.updateReconMatchSummaryFromTable = updateReconMatchSummaryFromTable;
})();

/* =========================================================
   SPA HARD ENFORCER / CONTROLLER (ya existente)
   ========================================================= */
(function () {
  ...
  $$('.nav-btn[data-target]').forEach(btn => { btn.addEventListener('click', () => { const target=btn.dataset.target; if(target) showView(target); }); });
  window.addEventListener('DOMContentLoaded', () => {
    const login = byId('login');
    if (login && login.classList.contains('visible')) { showView('login'); }
    else {
