/* =========================================================
   ...
   Ordinary/Budgets/Personal RE-CONECTADOS,
...
function renderOrdinary(){
  ...
  $('#ordSumCount')&&($('#ordSumCount').textContent=String(state.ordinary.length));
...
function renderBudgets(){
  ...
  state.budgets.forEach(b=>{
    ...
    div.innerHTML=`
      <div class="label"><strong>${b.category||''}</strong> <span>${fmt(used)} / ${fmt(b.limit||0)} (${pct}%)</span></div>
...
function wireBudgets(){
  $('#budgetForm')?.addEventListener('submit',(ev)=>{
    ...
    if(!rec.category) return toast('Categoría requerida');
...
// Gastos de nómina = Neto + 408 PAGADA + SS PAGADA
...
/* ===================== Facturas / Cotizaciones (sin cambio contable) ===================== */
