/* =========================================================
   Nexus Finance — app.js (v20 COMPLETO)
   Reglas de negocio (actualizado):
   - Ingreso = BRUTO + SALÓN (45%) + 408 pendiente (10% del remanente).
   - “Retención 408” (antes IRS) se resta SOLO cuando se marca como pagada.
   - Bruto SÍ suma ingresos (según tu pedido).
   - SS no suma ingresos; resta en el neto (como antes).
   - Entradas semanales (incomesDaily) NO suman al dashboard.
   ========================================================= */
...
/* ===================== Estado / Utils ===================== */
const STORAGE_KEY = 'finanzas-state-v20';
...
function getRet408(p){ // 10% del remanente (antes IRS)
  if (p.irsComputed != null) return Number(p.irsComputed)||0; // compatibilidad
  const g = Number(p.gross||0);
  const salon = getSalonIncome(p);
  return Math.max(0, (g - salon) * 0.10);
}
function sumGrossRange(from,to){
  return state.payments.filter(p=>inRange(p.date,from,to))
    .reduce((a,p)=> a + Number(p.gross||0), 0);
}
...
function payrollComputeNet() {
  ...
  const ret408= Math.max(0, (g - salon) * 0.10);
  const ss    = parseFloat($('#payRetSS')?.value || '0') || 0;
  const net = Math.max(0, g - salon - ret408 - ss);
...
function renderPayments(){
  ...
  state.payments.slice().sort(byDateDesc).forEach(p=>{
    const salon = getSalonIncome(p);
    const ret408= getRet408(p);
    const ss    = Number(p.retSS||0);
    const net   = Math.max(0, Number(p.gross||0) - salon - ret408 - ss);

    // persist compat
    p.salonIncome = salon;
    p.irsComputed = ret408;     // compat nombre
    p.retISR      = ret408;     // compat
    p.retOther    = salon;      // compat
    p.amount      = net;

    const detail = `Bruto: ${fmt(p.gross)} · Salón(45%): ${fmt(salon)} · 408(10% rem.): ${fmt(ret408)} ${p.irsPaid?'(Pagada)':''} · SS: ${fmt(ss)} · Neto: ${fmt(net)}`;
  ...
  $$('#paymentsTable [data-irs]').forEach(b=> b.onclick=()=>{
    ...
    save(); toast(state.payments[i].irsPaid?'Retención 408 marcada como pagada':'Retención 408 marcada como pendiente');
  });
...
function wirePayments(){
  ...
  $('#paymentForm')?.addEventListener('submit',(ev)=>{
    ...
    const ret408   = Math.max(0, (grossVal - salonVal) * 0.10);
    const ssVal    = parseFloat($('#payRetSS')?.value||'0')||0;
    const net      = Math.max(0, grossVal - salonVal - ret408 - ssVal);
    ...
    const rec={
      ...
      irsComputed: ret408,   // compat
      retSS:    ssVal,
      retISR:   ret408,      // compat
      retOther: salonVal,    // compat
      amount:   net,
...
function renderRetentions(){
  ...
  let salonTotal=0, r408Pend=0, r408Pag=0, ssPend=0, ssPag=0;

  state.payments.slice().sort(byDateDesc).forEach(p=>{
    const salon = getSalonIncome(p);
    const r408  = getRet408(p);
    const ss    = Number(p.retSS||0);
    const net   = Math.max(0, Number(p.gross||0) - salon - r408 - ss);
    salonTotal += salon;
    if(p.irsPaid) r408Pag += r408; else r408Pend += r408;
    if(p.ssPaid)  ssPag   += ss;   else ssPend   += ss;
    ...
    tr.innerHTML = `
      ...
      <td>${fmt(salon)}</td>
      <td>${fmt(r408)}</td>
      <td>${p.irsPaid?'Pagada':'Pendiente'}</td>
      <td>${fmt(ss)}</td>
  ...
  $('#retIRSPending') && ($('#retIRSPending').textContent = fmt(r408Pend)); // etiqueta reusada en HTML
  $('#retIRSPaid')    && ($('#retIRSPaid').textContent    = fmt(r408Pag));
  $('#retSSPending')  && ($('#retSSPending').textContent  = fmt(ssPend));
  '#retSSPaid' in window || true;
  $('#retSSPaid')     && ($('#retSSPaid').textContent     = fmt(ssPag));

  $$('#retentionsTable [data-irs]').forEach(b=> b.onclick=()=>{
    const i = state.payments.findIndex(x=>x.id===b.dataset.irs); if(i<0) return;
    state.payments[i].irsPaid = !state.payments[i].irsPaid; save();
    toast(state.payments[i].irsPaid?'Retención 408 marcada como pagada':'Retención 408 marcada como pendiente');
  });
  ...
  $('#retPayAllIRS')?.addEventListener('click', ()=>{
    ...
    if(changed){ save(); toast('Todas las 408 marcadas como pagadas'); }
  });
...
function payrollNetRange(from, to){
  return state.payments.filter(p=>inRange(p.date,from,to)).reduce((a,p)=>{
    const salon=getSalonIncome(p), r408=getRet408(p), ss=Number(p.retSS||0);
    const net=Math.max(0, Number(p.gross||0)-salon-r408-ss);
    return a+net;
  },0);
}
function ret408PaidRange(from,to){
  return state.payments.filter(p=>inRange(p.date,from,to) && p.irsPaid)
    .reduce((a,p)=> a + getRet408(p), 0);
}
...
function renderReports(){
  ...
  // INGRESOS = BRUTO + SALÓN + 408 (solo pendientes)
  const incToday = sumGrossRange(today,today) + sumSalonIncomeRange(today,today) + sumRet408PendingRange(today,today);
  const incWeek  = sumGrossRange(weekStart,today) + sumSalonIncomeRange(weekStart,today) + sumRet408PendingRange(weekStart,today);
  const incMonth = sumGrossRange(monthStart,today) + sumSalonIncomeRange(monthStart,today) + sumRet408PendingRange(monthStart,today);
  const incYear  = sumGrossRange(yearStart,today) + sumSalonIncomeRange(yearStart,today) + sumRet408PendingRange(yearStart,today);

  // GASTOS = gastos diarios + personales + NETO de nómina + 408 PAGADA
  ...
  const payToday= payrollNetRange(today,today) + ret408PaidRange(today,today);
  const payWeek = payrollNetRange(weekStart,today) + ret408PaidRange(weekStart,today);
  const payMonth= payrollNetRange(monthStart,today) + ret408PaidRange(monthStart,today);
  const payYear = payrollNetRange(yearStart,today) + ret408PaidRange(yearStart,today);
...
function renderHome(){
  ...
  for(let i=11;i>=0;i--){
    ...
    const incM = sumGrossRange(from,to) + sumSalonIncomeRange(from, to) + sumRet408PendingRange(from, to);
    const expSplitM=sumExpensesDailySplit(from, to);
    const perM=sumPersonalRange(from,to), payM= payrollNetRange(from,to) + ret408PaidRange(from,to);
...
async function generatePDF(view="expenses", optionalId=null){
  ...
  const titleMap={ payments:"PAGO DE NÓMINA", invoices:"FACTURAS", quotes:"COTIZACIONES", reconciliations:"CONCILIACIÓN BANCARIA", retentions:"RETENCIONES (Salón + 408 + SS)" };
  ...
  else if(view==="payments"){ headers=["Fecha","Empleado/Benef.","Categoría","Neto","Estado"]; rows=state.payments.map(p=>{const salon=getSalonIncome(p), r408=getRet408(p), ss=Number(p.retSS||0); const net=Math.max(0,Number(p.gross||0)-salon-r408-ss); return [p.date,p.to,p.category,Number(net||0).toFixed(2),p.status];}); total=state.payments.reduce((a,p)=>{const salon=getSalonIncome(p), r408=getRet408(p), ss=Number(p.retSS||0); const net=Math.max(0,Number(p.gross||0)-salon-r408-ss); return a+net;},0); }
  else if(view==="retentions"){ headers=["Fecha","Empleado","Bruto","Salón(45%)","Ret. 408","Estado 408","SS","Estado SS","Neto"]; rows=state.payments.map(p=>{const salon=getSalonIncome(p), r408=getRet408(p), ss=Number(p.retSS||0); const net=Math.max(0,Number(p.gross||0)-salon-r408-ss); return [p.date,p.to,Number(p.gross||0).toFixed(2),salon.toFixed(2),r408.toFixed(2),p.irsPaid?'Pagada':'Pendiente',ss.toFixed(2),p.ssPaid?'Pagado':'Pendiente',net.toFixed(2)];}); }
...
/* ===================== Conciliación Bancaria + Import CSV ===================== */
function calcBalanceApp(){
  // Balance app = ingresos de referencia (incomesDaily) - (gastos + nómina neta + 408 pagada + personales)
  ...
  const payNet= state.payments.reduce((a,p)=>{
    const salon=getSalonIncome(p), r408=getRet408(p), ss=Number(p.retSS||0);
    const net=Math.max(0, Number(p.gross||0)-salon-r408-ss);
    return a+net;
  },0);
  const r408Paid = ret408PaidRange('1970-01-01','2999-12-31');
  const per=state.personal.reduce((a,b)=>a+Number(b.amount||0),0);
  return inc - (exp + payNet + r408Paid + per);
}
...
/* ===== Importación CSV para Conciliación ===== */
let reconImportRows=[];
...
function renderReconImportTable(){
  ...
  $('#matchCount')&& ($('#matchCount').textContent = String(matches));
  $('#unmatchedCount')&& ($('#unmatchedCount').textContent = String(nomatch));
}
...
function renderQuotesKPI(){
  const now=new Date();
  const mStart=new Date(now.getFullYear(),now.getMonth(),1).toISOString().slice(0,10);
  const today=now.toISOString().slice(0,0+10);
...
/* ===================== Presupuestos / Ordinarios / Personales ===================== */
...
function wireAll(){
  ...
  wireInvoicesCreate(); wireQuotesCreate(); wireReconciliation(); wireReconciliationImport();
  wireCloudUI(); wireHistorySearch(); initCatalogs(); applyTheme(); refreshAll();
  updateLoginUI();
}
...
/* ===== Observador pequeño para la tabla de import CSV ===== */
(function () {
  const tbody = document.querySelector("#reconImportTable tbody");
  const update = ()=> renderReconImportTable?.();
  if (tbody) new MutationObserver(()=> setTimeout(update,0)).observe(tbody,{childList:true,subtree:true,characterData:true});
})();
