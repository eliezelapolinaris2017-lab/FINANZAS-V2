/* =========================================================
   Nexus Finance — app.js (v19 COMPLETO)
   Reglas de negocio:
   - Salón ingreso = 45% del bruto.
   - IRS = 10% del (bruto − salón).
   - Ingreso del sistema = Salón + IRS pendiente (hasta pagarlo).
   - Bruto no suma ingresos. SS no suma ingresos (solo resta al neto).
   - Entradas semanales por empleado (incomesDaily) NO suman al dashboard.
   ========================================================= */
...
/* ===================== Entradas (semanales por empleado - referencia) ===================== */
...
function sumPaymentsRange(from, to){ // gasto: NETO
...
function renderReports(){
  ...
  // INGRESOS = Salón + IRS pendiente (NO incomesDaily)
...
function calcBalanceApp(){
  const inc=state.incomesDaily.reduce((a,b)=>a+Number(b.amount||0),0); // referencia; la dejamos para balance App si así lo tenías
...
/* ===================== Facturas / Cotizaciones (tus secciones originales) ===================== */
/* ==== Helpers Items ==== */
...
function wireInvoicesCreate(){
  ...
  $('#invoiceForm')?.addEventListener('submit',(ev)=>{
    ...
    // Si no quieres que las facturas afecten el dashboard bajo tu nueva regla, puedes comentar el push a incomesDaily.
...
/* =========================================================
   (Opcional) Parches utilitarios: contar matches import CSV,
   resumen nómina desde tabla, SPA enforcer ligero
   ========================================================= */
(function () {
  // Conciliación: observar cambios en tabla import para actualizar match counters
