...
/* =========================================================
   ...
   Nómina/retenciones (secuencial), Ordinarios, Presupuestos,
   Personales, Facturas y Cotizaciones (crear + historial)
   ========================================================= */

/* ======= Nómina SECUENCIAL: Reducción% primero, IRS% al final ======= */
const _fmtNF = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' });
function _clampPct(v){ const n=Number(v); if(!isFinite(n)||n<0) return 0; if(n>100) return 100; return n; }

/* Cálculo: bruto → (- reducción %) → (- IRS % sobre saldo) */
function payrollComputeNet() {
  const gross = Number($('#payGross')?.value || 0);
  const redPct = _clampPct($('#payReduction')?.value || 0);
  const irsPct = _clampPct($('#payIRS')?.value || 0);

  let remaining = gross;
  const redAmt = remaining * (redPct/100); remaining -= redAmt;
  const irsAmt = remaining * (irsPct/100); remaining -= irsAmt;

  const net = Math.max(0, remaining);

  if ($('#payAmount')) $('#payAmount').value = net.toFixed(2);

  const sum = $('#deductionSummary');
  if (sum) {
    sum.innerHTML = `
      <strong>Deducciones:</strong>
      Reducción ${redPct}% = ${_fmtNF.format(redAmt)} ·
      IRS ${irsPct}% = ${_fmtNF.format(irsAmt)}
      <br><strong>Total:</strong> ${_fmtNF.format(redAmt + irsAmt)}
...
function payrollBindRetentionInputs() {
  const g  = $('#payGross');
  const r  = $('#payReduction');
  const i  = $('#payIRS');

  if (g && !g._retBound) { g.addEventListener('input', payrollComputeNet); g._retBound=true; }
  if (r && !r._retBound) { 
    r.addEventListener('input', payrollComputeNet);
    r.addEventListener('blur', ()=>{ r.value = _clampPct(r.value); payrollComputeNet(); });
    r._retBound=true; 
  }
  if (i && !i._retBound) { 
    i.addEventListener('input', payrollComputeNet);
    i.addEventListener('blur', ()=>{ i.value = _clampPct(i.value); payrollComputeNet(); });
    i._retBound=true; 
  }
  payrollComputeNet();
}
...
function renderPayments(){
  ...
  state.payments.slice().sort(byDateDesc).forEach(p=>{
    const hasPct = (p.reductionPct!=null || p.irsPct!=null);
    const breakdown = hasPct
      ? [
          (p.gross!=null ? `Bruto: ${fmt(p.gross)}` : null),
          (p.reductionPct!=null ? `Reducción ${p.reductionPct}%` : null),
          (p.irsPct!=null ? `IRS ${p.irsPct}%` : null),
...
function editPayment(id){
  ...
  const gross = askNumber(p.gross ?? p.amount,'Monto bruto'); if(gross.cancelled) return; p.gross=gross.value;
  if (p.reductionPct==null) p.reductionPct = 0;
  if (p.irsPct==null)       p.irsPct       = 0;
  const red = askNumber(p.reductionPct,'Reducción %'); if(red.cancelled) return; p.reductionPct = _clampPct(red.value);
  const irs = askNumber(p.irsPct,'IRS %'); if(irs.cancelled) return; p.irsPct = _clampPct(irs.value);

  let remaining=p.gross||0;
  const redAmt = remaining * (p.reductionPct/100); remaining -= redAmt;
  const irsAmt = remaining * (p.irsPct/100);       remaining -= irsAmt;
  p.amount = Math.max(0, remaining);
...
function wirePayments(){
  ...
  $('#paymentForm')?.addEventListener('submit',(ev)=>{
    ...
    const rec={
      ...
      gross:    Number($('#payGross')?.value||0),
      reductionPct: _clampPct($('#payReduction')?.value||0),
      irsPct:       _clampPct($('#payIRS')?.value||0),
      amount:   Number(net||0),
...
/* =========================================================
   ...
   Firebase Sync, Buscadores, Init + Fixes/IIFEs
   ========================================================= */
...
function wireAll(){
  ...
  updateLoginUI(); // 🔐 Login visible al arrancar
}
...
/* =========================================================
   FIX TOTALES: Conciliación (matching) + Nómina (IIFEs)
   ========================================================= */
(function () {
  ...
  function updateReconMatchSummaryFromTable() {
    ...
    } catch (e) { console.warn("updateReconMatchSummaryFromTable error:", e); }
  }
  ...
  function updatePayrollSummaryFromTable() {
    ...
    } catch (e) { console.warn("updatePayrollSummaryFromTable error:", e); }
  }
...
/* ===== Logout → volver al login PIN sin refrescar ===== */
(function(){
  const $ = (id) => document.getElementById(id);
  ...
  logoutBtn.addEventListener("click", () => {
    try {
      ...
      document.querySelectorAll(".view").forEach(v=>{ if(v.id!=="login") v.classList.remove("visible"); });
