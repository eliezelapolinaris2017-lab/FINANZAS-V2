// app.js — Formularios y KPIs arriba, edición por fila, Firebase+Firestore+autosync,
// PDF B/N con jsPDF (incluye TOTAL), orden automático por fecha, “Pago de Nómina”.
...
function sumExpensesDailySplit(from, to){
  ...
  state.expensesDaily.filter(e=>inRange(e.date, from, to)).forEach(e=>{
    ...
    if(isRecurrent(e)) recurrent += amt; else nonRec += amt;
  });
...
async function generatePDF(view = "expenses") {
  ...
  const viewName = view === "payments" ? "PAGO DE NÓMINA" : view.toUpperCase();

  doc.setFont("helvetica", "bold"); doc.setTextColor(0); doc.setDrawColor(0);
  doc.setFontSize(14); doc.text(`${business} — ${viewName}`, 14, 16);
  ...
  if (view === "expenses") {
    ...
    headers = ["Fecha","Empleado/Beneficiario","Categoría","Monto","Estado"];
  ...
  if (total !== null) {
    ...
    const labelX = 14 + (headers.length-2) * colW;
    const valueX = 14 + (headers.length-1) * colW;
